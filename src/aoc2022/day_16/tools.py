...
class Node(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def id(self) -> str:
        """Provide a string identifier unique to this Node."""
        raise NotImplementedError

    @property
    @abc.abstractmethod
    def parent(self) -> Optional["Node"]:
        """Provide the parent node of this Node (or None, if no parent)."""
        raise NotImplementedError

    @property
    @abc.abstractmethod
    def g(self) -> int:
        """Compute the cost for reaching this Node from the search start point."""
        raise NotImplementedError

    @property
    @abc.abstractmethod
    def h(self) -> int:
        """Estimate the cost for reaching the search goal from this Node."""
        raise NotImplementedError

    @abc.abstractmethod
    def get_successors(self) -> Iterable["Node"]:
        """List all nodes to search that are directly reachable from this Node."""
        raise NotImplementedError
    ...
    def lineage(self) -> list["Node"]:
        """List this Node and its recursive parents (if any)."""
        return [self] + (self.parent.lineage if self.parent else [])


def a_star_search(start: Node, goal_func: Callable) -> Node:
    ...
    raise ValueError("The search could not reach the end Node.")


class Room(Node):
    """Location within the volcano's tunnel network with a single Valve."""
    __slots__ = ["_name", "_parent", "_neighbours_map"]
    ...
    def parent(self) -> Optional["Room"]:
        """Provide the parent node of this Room (or None, if no parent)."""
        return self._parent
...
class Valve(Node):
    ...
    __slots__ = ["_name", "_open_time", "_parent", "_network", "output"]
    ...
    @property
    def parent(self) -> Optional["Valve"]:
        """Provide the parent node of this Valve (or None, if no parent)."""
        return self._parent
...
class DoubleValve(Node):
    ...
    @property
    def parent(self) -> Optional["DoubleValve"]:
        """Provide the parent node of this DoubleValve (or None, if no parent)."""
        return self._parent
